...
export type ClientMsg =
  | { t: 'hello'; name: string; heroId: string; modeId: string; room?: string }
  | { t: 'input'; mx: number; mz: number; ax: number; az: number; attack: boolean; super: boolean }
  | { t: 'ping'; at: number };

export type ServerMsg =
  | { t: 'welcome'; id: number; room: string; online: boolean }
  ...
  | { t: 'reward'; coins: number; xp: number; trophies: number }
  | { t: 'error'; msg: string }
  | { t: 'pong'; at: number };
